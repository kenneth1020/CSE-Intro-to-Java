...
public class elementalDiagnosis {
	...
	static double[] case4 = {0.60 , 0.30, 0.15, 0.10};
	static double[] case5 = {0.60 , 0.30, 0.15, 0.10, 0.05};
	static double[] case6 = {0.60 , 0.30, 0.15, 0.10, 0.05, 0.025};
	static double[] case7 = {0.60 , 0.30, 0.15, 0.10, 0.05 , 0.025, 0.05};
	static double[] case8 = {0.60 , 0.30, 0.15, 0.10, 0.05 , 0.025, 0.05, 0.01};
	...
	public static void answering(int arr[], int limit) {
		...
		while (counting < limit) {
			...
			if(answers.equalsIgnoreCase("yes")) {
				...
				System.out.println("Please put in number of choice. Choosing all " + limit + " choices or less");
				/**
				String num2 = scanner.toString();
				boolean state = checker(num2);
					if (state == false)
					{
						System.out.println("Sorry, word you have type seems to be out the limit. Resetting system: ");
						answering(arr, limit);
					}
	...
	//WIll print out the unsorted data of all the calculation from the elemental test
	public static void idE () {
		Element[] arr2;
		arr2 = new Element[27];
		System.out.println(" ");
		System.out.println("=====================================");
		System.out.println("Unsorted data from your personailty test");
		for(int i = 1 ; i < 28; i++) {
			arr2[i-1] = new Element(elementList[i], list.get(i-1).getAnswers());

			System.out.println(arr2[i-1].value + " " + arr2[i-1].element);
	...
	//Will print out the data for the sort elemental test the highest points firsts
	public static void sort(Element[] data) {
		int n = 27;
		double temp = 0;
		String temp2;
		for(int i=0; i < n; i++){
			for(int j=1; j < (n-i); j++){
				if(data[j-1].value < data[j].value){
					//swap the elements!
					temp = data[j-1].value;
					temp2 = data[j-1].element;
					data[j-1].value = data[j].value;
					data[j-1].element = data[j].element;
					data[j].value = temp;
					data[j].element = temp2;
				}
			}
		}//end of for loop
		System.out.println(" ");
		System.out.println("=====================================");
		System.out.println("Sorted data from your personailty test");
		for(int i = 0 ; i < 27; i++) {

			System.out.println(data[i].value + " " + data[i].element);
	...
	//switch cases. Depending on the variable that go input first will calculate the 
	//excel document data as the highest. The last variable interger getting put in 
	//last will score the lowest
	public static void switchesCases(int[] arr) {;
	int var = 0;
	double level[] = new double[28];
	//var is amount of input in the list
	//The switch cases goes up to 8.
	while(var < counting) {
		...
		//depending how much input is add. The data will be calculated.
		//max input is 8.
		switch(counting) {
		...
		case 8: for(int i = 1 ; i < 28 ; i++) {
			elementList[i] =	elementList[i] + level[i]*case8[var];
			//System.out.print(elementList[i]);
		}
		}//end of switch
		var++;
